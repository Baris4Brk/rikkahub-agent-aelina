...
object OwnerActionRegistry {
    val families: List<OwnerFamilySpec> = listOf(
        family(
            ...
            action("assistant_update", OwnerOperationRisk.REVERSIBLE_WRITE, "assistant_id, name?, system_prompt?, chat_model_id?, workspace_id?, enable_memory?, use_global_memory?, enable_recent_chats_reference?, stream_output?, fast_path_router_enabled?, enable_web_search?, generation_max_steps? (0=auto, 1..256), generation_turn_budget_minutes? (0=auto, 1..60)"),
        ...
        family(
            ...
            action("provider_delete", OwnerOperationRisk.IRREVERSIBLE, "provider_id, replacement_model_id?"),
            ...
            action("provider_model_list", OwnerOperationRisk.READ_ONLY, "provider_id"),
            action("provider_model_upsert", OwnerOperationRisk.REVERSIBLE_WRITE, "provider_id, definition"),
            action("provider_model_delete", OwnerOperationRisk.IRREVERSIBLE, "provider_id, model_id, replacement_model_id?"),
            action("provider_route_set", OwnerOperationRisk.REVERSIBLE_WRITE, "route, model_id?"),
        ),
        family(
        ...
        family(
            ...
            action("tts_library_delete", OwnerOperationRisk.IRREVERSIBLE, "artifact_id"),
            action("tts_stop", OwnerOperationRisk.REVERSIBLE_WRITE),
        ...
        family(
            ...
            action("workflow_create", OwnerOperationRisk.REVERSIBLE_WRITE, "definition{name, enabled?, trigger{type, params}, actions[1..32]{tool,args,timeout_seconds?}, conditions?}; actions must be non-empty"),
            action("workflow_update", OwnerOperationRisk.REVERSIBLE_WRITE, "definition{id,name, enabled?, trigger{type, params}, actions[1..32]{tool,args,timeout_seconds?}, conditions?}; actions must be non-empty"),
            ...
            action("schedule_list", OwnerOperationRisk.READ_ONLY),
            action("schedule_create", OwnerOperationRisk.REVERSIBLE_WRITE, "definition"),
            action("schedule_update", OwnerOperationRisk.REVERSIBLE_WRITE, "job_id, definition"),
            action("schedule_set_enabled", OwnerOperationRisk.REVERSIBLE_WRITE, "job_id, enabled"),
            action("schedule_run_now", OwnerOperationRisk.EXTERNAL_SIDE_EFFECT, "job_id"),
            action("schedule_delete", OwnerOperationRisk.IRREVERSIBLE, "job_id"),
            action("alarm_list", OwnerOperationRisk.READ_ONLY),
            action("alarm_create", OwnerOperationRisk.REVERSIBLE_WRITE, "definition"),
            action("alarm_update", OwnerOperationRisk.REVERSIBLE_WRITE, "alarm_id, definition"),
            action("alarm_set_enabled", OwnerOperationRisk.REVERSIBLE_WRITE, "alarm_id, enabled"),
            action("alarm_delete", OwnerOperationRisk.IRREVERSIBLE, "alarm_id"),
        ),
        family(
            OwnerToolFamily.UI,
        ...
        family(
            ...
            action("prompt_injection_upsert", OwnerOperationRisk.REVERSIBLE_WRITE, "definition{id?,name,enabled?,priority?,position?=after_system_prompt,content?,injectDepth?,role?=user}; camelCase injectDepth; role=user|assistant"),
            action("prompt_injection_delete", OwnerOperationRisk.IRREVERSIBLE, "injection_id"),
            ...
            action("lorebook_upsert", OwnerOperationRisk.REVERSIBLE_WRITE, "definition{id?,name,description?,enabled?,entries?[{id?,name,enabled?,priority?,position?,content?,injectDepth?,role?,keywords?,useRegex?,caseSensitive?,scanDepth?,constantActive?}]}"),
            action("lorebook_delete", OwnerOperationRisk.IRREVERSIBLE, "lorebook_id"),
        ),
        family(
            OwnerToolFamily.ASR,
